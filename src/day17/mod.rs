...
pub fn step2(input : &str) {
    ...
    println!("{}", factorized);
    let commands = factorized.chars().chain(vec!['n', '\n']).map(|c| c as i64).collect();
...
impl AsciiIo {
    ...
    fn find_path(&self) -> String {
        ...
        let mut pos : Coord = self.rows.iter().enumerate().find_map(|(i, r)| r.iter().enumerate().find_map(|(j, c)| match c {
            '^' | 'v' | '>' | '<' => Some(j),
            _ => None,
        }).map(|j| Coord{y:i, x:j})).unwrap();
        ...
        loop {
            ...
            if pos.forward(dir.left(), 1).and_then(|c| self.get_at_coord(&c)) == Some('#') {
                path.push_str("L,");
                dir = dir.left();
            } else if pos.forward(dir.right(), 1).and_then(|c| self.get_at_coord(&c)) == Some('#') {
                path.push_str("R,");
                dir = dir.right();
            } else {
                break;
...
impl Coord {
    fn forward(&self, dir : Direction, n : usize) -> Option<Coord> {
        match dir {
            Direction::North => if n <= self.y {Some(Coord{x: self.x, y: self.y - n})} else {None},
            Direction::West => if n <= self.x {Some(Coord{x: self.x - n, y: self.y})} else {None},
            Direction::East => Some(Coord{x: self.x + n, y: self.y}),
            Direction::South => Some(Coord{x: self.x, y: self.y + n}),
        }
    }
}
