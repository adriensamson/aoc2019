...
impl PathSegment {
    fn get_end(&self) -> Coord {
        match self.dir {
            Direction::Right => Coord {x: self.from.x + self.len, y: self.from.y},
            Direction::Left=> Coord {x: self.from.x - self.len, y: self.from.y},
            Direction::Up => Coord {x: self.from.x, y: self.from.y + self.len},
            Direction::Down => Coord {x: self.from.x, y: self.from.y - self.len},
        }
    }

    fn contains(&self, coord : &Coord) -> bool {
        match self.dir {
            Direction::Right => self.from.y == coord.y && self.from.x <= coord.x && coord.x <= self.from.x + self.len,
            Direction::Left => self.from.y == coord.y && self.from.x - self.len <= coord.x && coord.x <= self.from.x,
            Direction::Up => self.from.x == coord.x && self.from.y <= coord.y && coord.y <= self.from.y + self.len,
            Direction::Down => self.from.x == coord.x && self.from.y - self.len <= coord.y && coord.y <= self.from.y,
        }
    }
    ...
    fn intersection(&self, other : &Self) -> Vec<Coord> {
        let coords = match (self.dir, other.dir) {
            ...
            (Direction::Down, Direction::Left) => vec![Coord {x: self.from.x, y: other.from.y}],

            (Direction::Left, Direction::Up) |
            (Direction::Left, Direction::Down) |
            (Direction::Right, Direction::Up) |
            (Direction::Right, Direction::Down) => vec![Coord {x: other.from.x, y: self.from.y}],
            ...
            (Direction::Down, Direction::Down) => {
                if self.from.x != other.from.x {
                    ...
                    let x = self.from.x;
                    let ys = vec![self.from.y, self.from.y - self.len, self.from.y + self.len, other.from.y, other.from.y - other.len, other.from.y + other.len];
            ...
            (Direction::Left, Direction::Left) => {
                if self.from.y != other.from.y {
                    ...
                    let y = self.from.y;
                    let xs = vec![self.from.x, self.from.x - self.len, self.from.x + self.len, other.from.x, other.from.x - other.len, other.from.x + other.len];
                    let mut coords = Vec::new();
...
impl Coord {
    ...
    fn make_segment(&self, pstr : &str) -> PathSegment {
        ...
        PathSegment {from: *self, dir: Direction::from_str(pstr), len}
    }
}