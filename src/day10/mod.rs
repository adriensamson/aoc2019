...
pub fn step2(input : &str) {
    let asteroids = Map::parse(input);
    let max = asteroids.asteroids.iter()
    ...
    let mut asts : Vec<(usize, &Vector, &Coord)> = ast_by_dir.iter()
        .flat_map(|(dir, by_angle)| by_angle.iter()
            .enumerate()
            .map(move |(i, ba)| (i, dir, ba.1))
        ).collect();
    asts.sort_by(|a1, a2| a1.0.cmp(&a2.0).then(a1.1.angle().partial_cmp(&a2.1.angle()).unwrap()));
