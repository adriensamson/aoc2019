...
impl LevelCoord {
    ...
    fn around(&self) -> Vec<LevelCoord> {
        ...
        // Left
        match self.x {
            0 => res.push(LevelCoord {y: 2, x: 1, level: self.level - 1}),
            3 if self.y == 2 => {
                for y in 0..5 {
                    res.push(LevelCoord {y, x: 4, level: self.level + 1});
                }
            }
            _ => res.push(LevelCoord {y: self.y, x: self.x - 1, level: self.level}),
        }
        // Right
        match self.x {
            4 => res.push(LevelCoord {y: 2, x: 3, level: self.level - 1}),
            1 if self.y == 2 => {
                for y in 0..5 {
                    res.push(LevelCoord {y, x:0, level: self.level + 1});
                }
            }
            _ => res.push(LevelCoord {y: self.y, x: self.x + 1, level: self.level}),
        }
        // Top
        match self.y {
            0 => res.push(LevelCoord {y: 1, x: 2, level: self.level - 1}),
            3 if self.x == 2 => {
                for x in 0..5 {
                    res.push(LevelCoord {y: 4, x, level: self.level + 1});
                }
            }
            _ => res.push(LevelCoord {y: self.y - 1, x: self.x, level: self.level}),
        }
        // Bottom
        match self.y {
            4 => res.push(LevelCoord {y: 3, x: 2, level: self.level - 1}),
            1 if self.x == 2 => {
                for x in 0..5 {
                    res.push(LevelCoord {y: 0, x, level: self.level + 1});
                }
            }
            _ => res.push(LevelCoord {y: self.y + 1, x: self.x, level: self.level}),
        }
